...
function usesDictionary(encoding){
  if (encoding === 'PLAIN_DICTIONARY' || encoding === 'RLE_DICTIONARY' || encoding === undefined){
    return true
  } else {
    return false
  }
...
function decodePages(buffer, opts) {
  ...
  while (cursor.offset < cursor.size && (!opts.num_values || data.dlevels.length < opts.num_values)) {
    ...
    if (opts.dictionary && (usesDictionary(pageData.valueEncoding))) {
