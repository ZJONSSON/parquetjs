...
class ParquetEnvelopeWriter {
  ...
  constructor(schema, writeFn, closeFn, fileOffset, opts) {
    ...
    (opts.bloomFilters || []).forEach(bloomData => {
      this.bloomFilters[bloomData.column] = initSplitBlocks(128)
    });
  }
  ...
  writeBloomFilters(_rowGroups) {
    ...
    this.schema.fieldList.forEach( (c,i) => {
      rowGroups.forEach(group => {
        ...
        if (!column) return;
        if (column.name in this.bloomFilters === false) return;
        const blocks = this.bloomFilters[column.name];
        const bytes = blocks.length;

        // TODO: serialize headers
        // TODO: Concat biset
        const bloomFilterHeader = {
          numBytes: bytes,
          compression: 'UNCOMPRESSED'
        }
        
        // console.log("this.bloomFilters", this.bloomFilters)
        let bloomHeaders = parquet_util.serializeThrift(column.meta_data.columnIndex);
        // Buffer.concat(bloomHeaders, columnblocks)
...
function encodePages(schema, rowBuffer, opts) {
  ...
  for (let field of schema.fieldList) {
    ...
    // wip: perhaps there is another better place
    // console.log("field", opts)
    if (field.name in opts.bloomFilters) {
      const blocks = opts.bloomFilters[field.name];
      for (const value of values.values) {
          const hexHash = hash64(value, BUF_SEED);
          const longValue = Long.fromBytes(hexHash, true);
          filterInsert(blocks, longValue);
      }
    }
...
function encodeDataPageV2(column, rowCount, values, rlevels, dlevels, statistics) {
  ...
  // HERE is another place can take the hashes
...
function encodeColumnChunk(pages, opts) {
  ...
  // maybe we can add it here metadata.bloom_filter_offset = 
  // but prob not since we dont know what the offset is yet
...
function encodeRowGroup(schema, data, opts) {
  ...
  for (let field of schema.fieldList) {
    ...
    metadata.columns.push(cchunk);
     // cchunk.bloom_filter_offset = cchunkData.bloomFilterOffset
     // maybe here but prob not since filter is going to be written after
